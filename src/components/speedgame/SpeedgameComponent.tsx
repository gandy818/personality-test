...
import { fruitList, singerList, brandList, snackList, animalList } from '@/dataList/speedgame';
...
export default function SpeedgameComponent() {
  const [time, setTime, finish, setFinish] = useTimer(20);
    ...
    <div>
      {finish ? (
        ...
        >
          ...
          {/* 결과 목록 */}
          <div className="h-[466px] overflow-y-scroll bg-[#F7F6F3] flex flex-col gap-6 py-8 px-4 md:px-20">
            {resultList.map((item, index) => {
              return (
                <div key={index} className="flex justify-between">
                  <p className="text-2xl xs:text-[32px] font-semibold self-center">
                    {shuffledList[index]}
                  </p>
                  {item === 'PASS' ? (
                    <p className="text-[#FF5E66] flex gap-2 text-[32px] md:text-[40px] font-semibold text-nowrap">
                      {item}
                      <img src="/images/틀림.svg" className="w-8 md:w-full" />
                    </p>
                  ) : (
                    <p className="text-[#2EBA41] flex gap-2 text-[32px] md:text-[40px] font-semibold text-nowrap">
                      {item}
        ...
        <div>
          ...
          {/* 단어 */}
          <div className="text-center font-bold mt-10 mb-32 text-[80px] md:text-[150px]">
